...
class Cart(models.Model):
    ...
    def add_item(self, book_id, quantity, user):
        if self.items.filter(book_id=book_id):
            item = self.items.get(book_id=book_id)
            item.quantity += quantity
            item.save()
            return item

        item = self.items.create(book_id=book_id,
                                 quantity=quantity,
                                 updated_by=str(user),
                                 created_by=str(user))
    ...
    def remove_item(self, book_id):
        ...
        """
        try:
            cart_item = self.items.get(book_id=book_id)
            cart_item.delete()
    ...
    def update_item(self, book_id, quantity):
        """
        Update item quantity in database
        """
        self.items.filter(book_id=book_id).update(quantity=quantity)

    def get_items(self):
        return self.items.prefetch_related('book', 'book__pics').all()

    def get_items_with_taxes(self):

        return self.items.prefetch_related('book', 'book__tax').all()
...
class CartItem(models.Model):
    ...
    def get_taxes(self):
        book = self.book
        if book.tax:
            return book.tax.calculate(book.price, self.quantity)

        return 0.0

    def get_shipping_cost(self):
        book = self.book
        if book.is_free_shipping:
            return 0.0

        return float(book.shipping_cost) * float(self.quantity)
...
class OrderManager(models.Manager):
    def place(self, cart_id, billing_address_id, shipping_address_id, payment_method,
        ...
        for item in cart.get_items_with_taxes():
            ...
            if book.tax:
                tax_rate = book.tax.rate
                tax_method = book.tax.method
            else:
                tax_rate = 0.0
                tax_method = None
...
class OrderItem(models.Model):
    ...
    price = models.DecimalField(max_digits=9, decimal_places=2, help_text='Unit price of the book')
